...
- `live · fast path` — результат получен без создания diagnostic PNG;
- `live · diagnostic frame` — текущий inference дополнительно обновил overlay, ONNX preview и выпрямленную шкалу.

Частота тяжёлых preview настраивается как каждый 1-й, 2-й, 4-й или 8-й обработанный кадр; по умолчанию используется каждый 4-й. Это не снижает частоту решений detector: между diagnostic-кадрами выполняется полный ONNX inference и geometry gate, но пропускаются perspective preview и PNG-кодирование. Отключение всех preview полностью замораживает последние изображения и оставляет только fast path. Настройки отдельных preview и интервала сохраняются между запусками. Live capture не отправляет ввод и не взаимодействует с процессом VRChat.
