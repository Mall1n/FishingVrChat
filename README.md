...
Diagnostic preview ограничен примерно четырьмя обновлениями в секунду. Это не снижает частоту решений detector: между diagnostic-кадрами выполняется полный ONNX inference и geometry gate, но пропускаются perspective preview и PNG-кодирование. Live capture не отправляет ввод и не взаимодействует с процессом VRChat.
