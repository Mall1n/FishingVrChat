...
В правом блоке `ONNX detector` выбери экспортированный файл и включи detector. Приложение выполняет тот же letterbox `1024 × 1024`, читает output в формате `[x, y, w, h, confidence, class, angle]` и применяет deployment gate из раздела Test. Для GPU inference используется DirectML; отдельная установка CUDA Runtime для приложения не требуется.
