...
- `precision` — сколько найденных рамок действительно являются рамками; выше = меньше ложных срабатываний.
- `recall` — сколько настоящих рамок модель нашла; выше = меньше пропусков.
- `mAP50` — качество детекции при более мягком совпадении OBB.
- `mAP50-95` — более строгая итоговая метрика; на неё стоит ориентироваться сильнее.
- `val/*_loss` — вспомогательные ошибки на Validation; должны в целом снижаться, но сами по себе не означают хорошую модель.

Для окончательной оценки не опирайся только на Train/Validation: запусти отдельную проверку Test с выбранным `best.pt`.
